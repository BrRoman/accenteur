...
# into 3 JSON Objects (models, roots and terminations) in the global data file 'data.json'.
    ...
    for c in range(len(word) - 2):
        if word[c] in vowels:
            if (word[c + 1] in consonantics and word[c + 2] in consonantics) and not (word[c + 1] in begadkefat and word[c + 2] in liquids):
                # If "āe" or "ōe" or "āu", don't set word[c] long.
                if not ((word[c] in ["e", "u"]) and (word[c - 1] in ["ā", "ō"])):
                    word = word[:c] + \
                        longs[vowels.index(word[c])] + word[c + 1:]
    return word
    ...
    if not(l.startswith("!")):
        ...
        # Roots (roots[num of the root] = [characters to delete, characters to add]):
            ...
            if l.split(":")[2] == "K":
                ...
                roots_tmp[int(l.split(":")[1])] = [l.split(
                    ":")[2].split(",")[0], l.split(":")[2].split(",")[1]]
        ...
        # Terminations (terms[num] = [num_rad, termination]):
            ...
            for s in subranges:
                if "-" in s:
                    # "for t in range(1, 6)" (if range = "1,5").
                    for t in range(int(s.split("-")[0]), int(s.split("-")[1]) + 1):
                        if not(add_term):
                            # If the model doesn't inherit from a father, we create a terminations' list.
                            terms_tmp[str(t)] = []
                        if rm_term:
                            del terms_tmp[str(t)]
                        else:
        ...
        # and we reinitialize the data:
            ...
            models[key] = dict(roots=deepcopy(roots_tmp),
                               terms=deepcopy(terms_tmp))
            key = ""
            father = ""
            terms_tmp.clear()
            roots_tmp.clear()
            ...
            if sub_term != "-":
                for term_quantified in sub_term[1].split(","):
                    if atone(term_quantified) in terminations:
                        terminations[atone(term_quantified)].append(
                            [term_quantified, m, int(sub_term[0])])
                    else:
...
for l in lemmes_lines:
    if not (l.startswith("!") or l == ""):
        ...
        for canonical in canonicals:
            canonical = canonical[0:-1] if (canonical.endswith(
                "2") or canonical.endswith("3") or canonical.endswith("4")) else canonical
            for c in canonical.split(","):
                for num_root in model["roots"]:
                    if splinters[1] == "inv":
                        ...
                        root0 = (c[0:-int(del_part)] if del_part !=
                                 0 else c) + (add_part if add_part != "0" else "")
                    if not (atone(root0) in roots):
                        roots[atone(root0)] = []
                    # Append a new root:
                    roots[atone(root0)].append(
                        [long_by_position(root0), splinters[1], num_root])
        ...
        if splinters[2] != '':
            for splinter in splinters[2].split(","):
                ...
                roots[atone(splinter)].append(
                    [long_by_position(splinter), splinters[1], 1])
        if splinters[3] != '':
            for splinter in splinters[3].split(","):
                if not (atone(splinter) in roots):
                    roots[atone(splinter)] = []
                roots[atone(splinter)].append(
                    [long_by_position(splinter), splinters[1], 2])
